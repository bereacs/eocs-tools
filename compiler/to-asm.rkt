...
(define (to-asm input)
  (cond
    ...
    [(number? (id-value (first input)))
     (string-append (asm-num (first input)) (to-asm (rest input)))
     ]
    [(binop? (id-value (first input)))
     (string-append (asm-binop (first input)) (to-asm (rest input)))
     ]
    [(label? (id-value (first input)))
     (string-append (asm-label(first input)) (to-asm (rest input)))
     ]
    [(goto? (id-value (first input)))
     (string-append (asm-goto (first input)) (to-asm (rest input)))
     ]
    [(jump? (id-value (first input)))
     (string-append (asm-jump (first input)) (to-asm (rest input)))
     ]
    [(symbol? (id-value (first input)))
     (string-append (asm-symbol (first input)) (to-asm (rest input)))
     ]
